...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (editing) {
      setFormValue(members.find((member) => member.id === editing));
    } else {
      setFormValue({
        id: null,
        fname: "",
  ...
  const onChange = (evt) => {
    ...
    setFormValue({ ...formValue, [evt.target.id]: evt.target.value });
  };
  const edit = (id) => {
    ...
    setEditing(parseInt(id.target.id));
  };
  const submitNewMember = () => {
    ...
    setMembers(members.concat(formValue), (formValue.id = members.length + 1));
  };
  const editExistingMember = () => {
    ...
    let editedMember = members.map((member) => {
      if (member.id === formValue.id) {
        return formValue;
      } else {
        return member;
      }
